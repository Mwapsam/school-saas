#!/usr/bin/env python3
"""Migrate Phase B (13 LEGACY fees templates) to new layout system."""

import re
from pathlib import Path

PROJECT_ROOT = Path(__file__).parent.parent
TEMPLATES_DIR = PROJECT_ROOT / "templates" / "core" / "fees"

# Phase B LEGACY fees files (layout type inferred from name pattern)
PHASE_B_FILES = {
    "batch_assignment": {"layout": "list.html", "icon": "fa-money-check-alt"},
    "batch_report": {"layout": "list.html", "icon": "fa-chart-bar"},
    "discounts": {"layout": "list.html", "icon": "fa-percent"},
    "fine_slabs": {"layout": "list.html", "icon": "fa-sliders-h"},
    "particulars": {"layout": "list.html", "icon": "fa-list"},
    "payment_agreement_detail": {"layout": "detail.html", "icon": "fa-file-contract"},  # Done manually
    "payment_agreement_form": {"layout": "form.html", "icon": "fa-file-contract"},  # Done manually
    "payment_agreement_list": {"layout": "list.html", "icon": "fa-file-contract"},  # Done manually
    "receipt_reconciliation": {"layout": "list.html", "icon": "fa-sync-alt"},
    "statistics_report": {"layout": "list.html", "icon": "fa-chart-pie"},
    "student_statement": {"layout": "list.html", "icon": "fa-receipt"},
    "waivers": {"layout": "list.html", "icon": "fa-ban"},
    "year_reports": {"layout": "list.html", "icon": "fa-file-alt"},
}
...
def main():
    print("\n[MIGRATION] Phase B Fees LEGACY Template Migration")
    print(f"   Target: {len(PHASE_B_FILES)} files")

    success = 0
    for filename, config in PHASE_B_FILES.items():
        # Skip already-migrated files
        if filename in ["payment_agreement_detail", "payment_agreement_form", "payment_agreement_list"]:
            print(f"  SKIP {filename}.html (already done)")
            continue

        if migrate_file(filename, config):
            success += 1

    print(f"\n[RESULT] Migrated {success}/{len(PHASE_B_FILES) - 3} remaining files")
    return 0 if success == len(PHASE_B_FILES) - 3 else 1
